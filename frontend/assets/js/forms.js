  ...
  const formConfigs = {
    'business-form': {
      ...
      validate(values, form) {
        let ok = true;
        if (values.companyName.trim() && values.companyName.trim().length < 2) { setError($('#companyName', form), 'Enter a valid company name'); ok = false; }
        if (values.companyName.trim() && /^\d+$/.test(values.companyName.trim())) { setError($('#companyName', form), 'Company name cannot be only numbers'); ok = false; }
