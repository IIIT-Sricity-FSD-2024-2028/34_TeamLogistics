...
(function(){
  ...
  document.querySelectorAll('[data-mark-read]').forEach(btn=>btn.addEventListener('click',()=>{ alert('All notifications marked as read'); }));
  document.querySelectorAll('[data-download]').forEach(btn=>btn.addEventListener('click',(e)=>{e.preventDefault(); alert('Prototype: download started');}));
  ...
  if(payForm){ payForm.addEventListener('submit',function(e){ e.preventDefault(); alert('Transaction submitted successfully'); location.href='invoices.html'; }); }
