(function(){
  ...
  function bindForms(){const noteForm=document.getElementById('noteForm'); if(noteForm){noteForm.addEventListener('submit', function(e){e.preventDefault(); const noteId=Number(document.getElementById('noteId').value||0)||null; const text=document.getElementById('noteText').value.trim(); if(!text) return alert('Enter a note first.'); upsertNote(detailsOrder().id,noteId,text)}); document.getElementById('clearNoteBtn').onclick=function(){document.getElementById('noteId').value=''; document.getElementById('noteText').value='';};}
    ...
    if(profileForm){
      ...
      profileForm.addEventListener('submit', function(e){
        ...
        if(!/^[A-Za-z ]+$/.test(fullName) || fullName.length < 2) return alert('Full name should contain only letters and spaces.');
        if(!emailRegex.test(email)) return alert('Enter a valid email address.');
        if(!/^[6-9]\d{9}$/.test(phone)) return alert('Phone number should be 10 digits and start with 9, 8, 7, or 6.');
        if(!/^[A-Z]{2}-\d{2}-\d{4}-\d{7}$/.test(license)) return alert('License number should be in AA-12-1234-1234567 format.');
    ...
    const messageForm=document.getElementById('messageForm'); if(messageForm){messageForm.addEventListener('submit', function(e){e.preventDefault(); const to=document.getElementById('msgTo').value.trim()||'Dispatch',subject=document.getElementById('msgSubject').value.trim(),body=document.getElementById('msgBody').value.trim(); if(!subject||!body) return alert('Please fill subject and message.'); state.messages.unshift({id:Date.now(),from:`To: ${to}`,subject,body,time:'Just now'}); save(state); alert('Message sent in prototype.'); render();});}
  ...
  const logoutBtn=document.getElementById('driverLogout'); if(logoutBtn){logoutBtn.onclick=function(){localStorage.removeItem('driverLoggedIn'); sessionStorage.removeItem('driverLoggedIn'); location.href='../login.html';};}
