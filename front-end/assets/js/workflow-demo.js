...
(function () {
  ...
  function renderSuperuserReportsAndNotifications() {
    const isTransactions = location.pathname.endsWith('/superuser/transactions.html');
    if (!isTransactions) return;

    const root = document.querySelector('#pageRoot') || document.body;
    if (!root || $('[data-workflow-finance="1"]')) return;
    ...
    panel.setAttribute('data-workflow-finance', '1');
    panel.style.cssText = 'margin-top:16px;padding:14px;border:1px solid rgba(255,255,255,0.08);border-radius:16px;background:rgba(255,255,255,0.02)';

    const unpaidList = unpaid
      .map(
        (i) => `<li style="margin:6px 0;display:flex;justify-content:space-between;gap:12px;align-items:center">
          <div><strong>${i.id}</strong> • ${currency(i.amount)} • Order ${i.orderId}</div>
          <button class="btn-dark" style="padding:8px 10px;border-radius:10px" data-remind="${i.id}">Notify Client</button>
        </li>`
      )
      .join('');
    ...
    const txnRows = txns
      .map(
        (t) => `<tr>
          <td style="padding:8px 6px">${t.invoiceId}</td>
          <td style="padding:8px 6px">${t.date}</td>
          <td style="padding:8px 6px">${t.mode}</td>
          <td style="padding:8px 6px">${(t.txnId && t.txnId !== t.mode) ? t.txnId : (t.transactionId || '--')}</td>
          <td style="padding:8px 6px">${currency(t.amount)}</td>
    ...
    panel.innerHTML = `
      <div style="font-weight:900;font-size:18px">Billing & Payments</div>
      <div style="color:#b5b5b5;font-size:13px;margin-top:4px">When a delivery is marked <strong>Delivered</strong> by the driver, an invoice is generated for the business client. If payment is not submitted, Super User can send a reminder.</div>
      <div style="margin-top:14px;display:grid;grid-template-columns:1fr;gap:14px">
        <div>
          <div style="font-weight:800;margin-bottom:6px">Unpaid invoices</div>
          ${unpaid.length ? `<ul style="padding-left:18px">${unpaidList}</ul>` : `<div style="color:#9ca3af">No unpaid invoices.</div>`}
        </div>
        <div>
          <div style="font-weight:800;margin-bottom:6px">Submitted transactions</div>
          <div style="overflow:auto">
            <table style="width:100%;border-collapse:collapse;min-width:720px">
              <thead><tr style="color:#9ca3af;font-size:12px;text-align:left">
                <th style="padding:8px 6px;border-bottom:1px solid rgba(255,255,255,0.08)">Invoice</th>
                <th style="padding:8px 6px;border-bottom:1px solid rgba(255,255,255,0.08)">Date</th>
                <th style="padding:8px 6px;border-bottom:1px solid rgba(255,255,255,0.08)">Mode</th>
                <th style="padding:8px 6px;border-bottom:1px solid rgba(255,255,255,0.08)">Transaction Id</th>
                <th style="padding:8px 6px;border-bottom:1px solid rgba(255,255,255,0.08)">Amount</th>
                <th style="padding:8px 6px;border-bottom:1px solid rgba(255,255,255,0.08)">Status</th>
              </tr></thead>
              <tbody>${txnRows || `<tr><td colspan="6" style="padding:10px 6px;color:#9ca3af">No transactions submitted yet.</td></tr>`}</tbody>
            </table>
          </div>
        </div>
      </div>
    ...
    $$('[data-remind]', panel).forEach((btn) => {
      btn.addEventListener('click', () => {
        const invId = btn.getAttribute('data-remind');
        const notifs = getNotifs();
        notifs.unshift({
          id: `N-${Date.now()}`,
          to: 'business-client',
          title: 'Payment Reminder',
          message: `Please submit transaction details for invoice ${invId}.`,
          createdAt: nowISO(),
          read: false
        });
        saveNotifs(notifs);
        try {
          window.dispatchEvent(new CustomEvent('ds-notifications-updated'));
        } catch (e) {}
        wfToast(`Business client notified for unpaid invoice ${invId}.`);
      });
    });
  }
