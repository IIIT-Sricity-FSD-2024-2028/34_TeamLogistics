...
window.FleetManagerData.applyFleetManagerProfile = function(){
  ...
  document.querySelectorAll('#fm-email').forEach(function(el){ if(document.activeElement !== el) el.value = email; });
  document.querySelectorAll('#fm-phone').forEach(function(el){ if(document.activeElement !== el) el.value = phone; });
  document.querySelectorAll('#fm-company-name').forEach(function(el){ if(document.activeElement !== el) el.value = companyName; });
  document.querySelectorAll('#fm-company-address').forEach(function(el){ if(document.activeElement !== el) el.value = companyAddress; });
  document.querySelectorAll('#fm-number-of-vehicles').forEach(function(el){ if(document.activeElement !== el) el.value = numberOfVehicles; });

  var heroHeading = document.querySelector('.hero h2');
  if(heroHeading && /welcome back/i.test(heroHeading.textContent || '')){
    heroHeading.textContent = 'Welcome Back, ' + fullName;
  }

  document.querySelectorAll('[data-fm-full-name]').forEach(function(el){ el.textContent = fullName; });
  document.querySelectorAll('[data-fm-company-name]').forEach(function(el){ el.textContent = companyName; });
  document.querySelectorAll('[data-fm-company-address]').forEach(function(el){ el.textContent = companyAddress; });
  document.querySelectorAll('[data-fm-email]').forEach(function(el){ el.textContent = email; });
  document.querySelectorAll('[data-fm-phone]').forEach(function(el){ el.textContent = phone; });
  document.querySelectorAll('[data-fm-vehicle-count]').forEach(function(el){ el.textContent = numberOfVehicles; });
};
